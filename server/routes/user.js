    ...
    .then((doc) => {
      ...
      if (!user.invitationToken.length) {
        ...
        return transporter.sendMail({
          ...
          // Production
          html: `<br/><br/><strong>Welcome to Eximius</strong><br/> <p>You have been invited by ${user.username} to join us. </p> <br/> <p> ${message} </p> <br/> <br/> <em>Click on the following link to accept your invitation.</em> <br/> <a href="https://eximius.herokuapp.com/authentication/sign-up/${token}">Accept your invitation</a> <br/><br/> <p>${user.username} & Eximius Team,<p/> <p>See you around<p/>`
        });
      }
    })
    .then((emailResult) => {
...
UserRouter.post('/profile/contact-us', (req, res, next) => {
  const { email, message } = req.body;
  ...
  transporter
    .sendMail({
      ...
      html: `<strong>This email: ${email}</strong><br/>  <p>Sends the following message: </p> <br/> <p> ${message} </p>`
    })
    .then((emailResult) => {
      console.log('RESULT OF SENDING EMAIL ->', emailResult);
      res.json({ emailResult });
